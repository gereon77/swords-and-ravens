...
export default class CombatInfoComponent extends Component<CombatInfoComponentProps> {
  ...
  render(): ReactNode {
    const showVsb =
      this.attacker.valyrianSteelBlade > 0 ||
      this.defender.valyrianSteelBlade > 0;
    const showTob =
      this.attacker.tidesOfBattleCard !== undefined ||
      this.defender.tidesOfBattleCard !== undefined;
    ...
    return (
      <div style={{ maxWidth: "336px", minWidth: "336px" }}>
        <div
          style={{
            display: "grid",
            gridGap: "5px",
        ...
        >
          <div style={{ gridRow: "1", gridColumn: "1" }}>
            {this.attacker.isWinner && (
              <img
                src={knightBannerImage}
                width="28"
                style={{
                  marginRight: 2,
                  marginBottom: 5,
            ...
            <b
              style={{ color: this.attacker.house.color, fontSize: "1.25rem" }}
            >
              {this.attacker.house.name}
            </b>
            {this.attacker.isWinner && (
              <img
                src={knightBannerImage}
          ...
          <div style={{ gridRow: "1", gridColumn: "2" }} className="text-right">
            ...
            <b
              style={{ color: this.defender.house.color, fontSize: "1.25rem" }}
            >
              {this.defender.house.name}
            </b>
            {this.defender.isWinner && (
              <img
                src={knightBannerImage}
        ...
        >
          ...
          <div style={{ gridRow: "2", gridColumn: "1 / span 2" }}>
            {this.attacker.armyUnits.map((ut, i) => (
              <UnitIconComponent
                key={`combat-info-units-${this.attacker.house.id}_${i}`}
          ...
          <div style={{ gridRow: "2", gridColumn: "4 / span 2" }}>
            {this.defender.armyUnits.map((ut, i) => (
              <UnitIconComponent
                key={`combat-info-units-${this.defender.house.id}_${i}`}
            ...
            {this.defender.woundedUnits.map((ut, i) => (
              <UnitIconComponent
                key={`combat-info-wounded-units-${this.defender.house.id}_${i}`}
                house={this.defender.house}
                unitType={ut}
                size="smedium"
                makeGreyjoyUnitsBlack={true}
          ...
          <div
            ...
            className={showTob ? "" : "display-none"}
          >
            {this.attacker.tidesOfBattleCard
              ? this.attacker.tidesOfBattleCard.combatStrength
              : 0}
          </div>
          <div
          ...
          >
            {this.defender.tidesOfBattleCard
              ? this.defender.tidesOfBattleCard.combatStrength
              : 0}
          </div>

          <div
            style={{ gridRow: "7", gridColumn: "2" }}
            className={showVsb ? "" : "display-none"}
          >
          ...
          <div
            style={{ gridRow: "7 / span 3", gridColumn: "1" }}
            className={showTob ? "" : "display-none"}
          >
            {this.attacker.tidesOfBattleCard ? (
              <TidesOfBattleCardComponent
                tidesOfBattleCard={this.attacker.tidesOfBattleCard}
              />
            ) : (
          ...
          <div
            style={{ gridRow: "7 / span 3", gridColumn: "5" }}
            className={showTob ? "" : "display-none"}
          >
            {this.defender.tidesOfBattleCard ? (
              <TidesOfBattleCardComponent
                tidesOfBattleCard={this.defender.tidesOfBattleCard}
              />
            ) : (
              <div className="vertical-game-card game-card-slot tiny" />
            )}
