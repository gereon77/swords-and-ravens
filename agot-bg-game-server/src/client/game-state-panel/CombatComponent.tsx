...
import ImmediatelyHouseCardAbilitiesResolutionGameState from "../../common/ingame-game-state/action-game-state/resolve-march-order-game-state/combat-game-state/immediately-house-card-abilities-resolution-game-state/ImmediatelyHouseCardAbilitiesResolutionGameState";
import ImmediatelyHouseCardAbilitiesResolutionComponent from "./house-card-abilities/ImmediatelyHouseCardAbilitiesResolutionComponent";
import CancelHouseCardAbilitiesGameState from "../../common/ingame-game-state/action-game-state/resolve-march-order-game-state/combat-game-state/cancel-house-card-abilities-game-state/CancelHouseCardAbilitiesGameState";
...
export default class CombatComponent extends Component<
  GameStateComponentProps<CombatGameState>
> {
  ...
  render(): ReactNode {
    ...
    const winner: House | null =
      winners.length > 0 ? this.combat.game.houses.get(winners[0].house) : null;
    const houseCombatDatas =
      this.combatStats.length > 0
        ? this.combatStats.map((stat) => {
            const house = this.combat.game.houses.get(stat.house);
            const houseCard = stat.houseCard
              ? this.getHouseCard(stat.houseCard)
              : null;
        ...
        : [
            ...
            },
            {
              ...
              armyUnits: this.combat.defendingArmy
                .filter((u) => !u.wounded)
                .map((u) => u.type),
              woundedUnits: this.combat.defendingArmy
                .filter((u) => u.wounded)
                .map((u) => u.type),
  ...
  private getHouseCardBackId(house: House): string | undefined {
    ...
    if (combatData.houseCardChosen) {
      return this.combat.ingameGameState.isVassalHouse(house)
        ? "vassal"
        : house.id;
    }
  ...
  modifyUnitsOnMap(): [Unit, PartialRecursive<UnitOnMapProperties>][] {
    ...
    const postCombat =
      this.combat.childGameState instanceof PostCombatGameState
        ? this.combat.childGameState
        : null;
    const drawMarchMarkers =
      !postCombat ||
      (!postCombat.hasChildGameState(AfterCombatHouseCardAbilitiesGameState) &&
    ...
    if (
      !selectUnits ||
      !this.props.gameClient.doesControlHouse(selectUnits.house)
    ) {
      return this.combat.attackingArmy.map((u) => [
        u,
        {
