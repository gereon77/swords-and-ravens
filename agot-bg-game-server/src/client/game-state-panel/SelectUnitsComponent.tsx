...
@observer
export default class SelectUnitsComponent extends Component<
  GameStateComponentProps<SelectUnitsGameState<any>>
> {
        ...
        <Col xs={12}>
          {this.props.gameClient.doesControlHouse(
                  ...
                  <div>
                    ...
                    <ul className="pl-0">
                      {this.selectedUnits.entries.map(([region, units]) => (
                        <li key={`select-units_${region.id}`}>
                          {joinReactNodes(
                            units.map((u, i) => (
                              <b key={`select-units_${region.id}_${u.id}_${i}`}>
                                {u.type.name}
                              </b>
                            )),
                            ", "
              ...
              <Row className="justify-content-center">
                <Col xs="auto">
                  <Button
                    ...
                    disabled={
                      !this.props.gameState.selectedCountMatchesExpectedCount(
                        this.selectedUnits.entries
                      )
                    }
                  >
                    Confirm
                ...
                <Col xs="auto">
                  <Button
                    ...
                    onClick={() => this.reset()}
                    variant="danger"
                    disabled={this.selectedUnits.size == 0}
                  >
                    Reset
                  </Button>
                </Col>
            ...
            <div className="text-center">
              Waiting for{" "}
              {
                this.props.gameState.ingame.getControllerOfHouse(
                  this.props.gameState.house
                ).house.name
              }
              ...
  ...
  confirm(): void {
    if (this.selectedUnits.size == 0) {
      if (
        !window.confirm("You haven't selected any unit yet. Continue anyway?")
      ) {
        return;
      }
    }
    ...
    if (
      this.props.gameState.canBeSkipped &&
      this.selectedUnits.size < this.props.gameState.count
    ) {
      if (
        !window.confirm(
          "You haven't selected all possible units yet. Continue anyway?"
        )
      ) {
        return;
      }
  ...
  getSelectableUnits(): Unit[] {
    let result =
      this.countSelectedUnits() == this.props.gameState.count
        ? _.flatMap(this.selectedUnits.values)
        : [...this.props.gameState.possibleUnits];

    if (
      this.props.gameState.selectedUnitsMustBeOfSameRegion &&
      this.selectedUnits.size > 0
    ) {
  ...
  modifyUnitOnMap(): [Unit, PartialRecursive<UnitOnMapProperties>][] {
    if (this.props.gameClient.doesControlHouse(this.props.gameState.house)) {
      ...
      return this.getSelectableUnits().map(
        (u) =>
          [
            u,
            {
              highlight: {
                active: true,
                color: selectedUnits.includes(u) ? "yellow" : "white"
              },
  ...
  componentDidMount(): void {
    this.props.mapControls.modifyUnitsOnMap.push(
      (this.modifyUnitOnMapCallback = () => this.modifyUnitOnMap())
    );
  }
