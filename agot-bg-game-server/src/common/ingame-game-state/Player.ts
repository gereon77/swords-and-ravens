...
export default class Player {
  ...
  get isNeededForVote(): boolean {
    return (
      this.user.entireGame.ingameGameState?.votes.values
        .filter((vote) => vote.state == VoteState.ONGOING)
        .some((vote) => !vote.votes.has(this.house)) ?? false
    );
  }
  ...
  sendPbemResponseTime(): void {
    ...
    if (
      ...
      !this.waitedForData.hasBeenReactivated
    ) {
      this.user.entireGame.onNewPbemResponseTime(
        this.user,
    ...
    return {
      ...
      liveClockData: this.liveClockData
        ? {
            ...
            timerStartedAt: this.liveClockData.timerStartedAt
    ...
    return new Player(
      ...
      data.liveClockData
        ? {
            ...
            serverTimer: null,
            timerStartedAt: data.liveClockData.timerStartedAt
