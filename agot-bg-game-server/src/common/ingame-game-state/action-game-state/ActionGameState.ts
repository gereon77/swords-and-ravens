...
export default class ActionGameState extends GameState<
  ...
  | ReconcileArmiesGameState<ActionGameState>
  | ScoreObjectivesGameState
> {
  ...
  onUseRavenGameStateEnd(): void {
    ...
    this.ordersOnBoard.keys.forEach((region) => {
      const order = this.ordersOnBoard.get(region);
      if (
        this.game.isOrderRestricted(region, order, this.planningRestrictions)
      ) {
        this.removeOrderFromRegion(region);
      }
    });
  ...
  getPossibleSupportingRegions(
    ...
    return (
      this.game.world
        .getNeighbouringRegions(attackedRegion)
  ...
  static deserializeFromServer(
    ...
    actionGameState.planningRestrictions = data.planningRestrictions
