...
export default class MasterAtArmsGameState extends GameState<
  ExecuteLoanGameState,
  SelectRegionGameState<MasterAtArmsGameState>
> {
  ...
  serializeToClient(
    admin: boolean,
    player: Player | null
  ): SerializedMasterAtArmsGameState {
    return {
      type: "master-at-arms",
  ...
  static deserializeFromServer(
    ...
    data: SerializedMasterAtArmsGameState
  ): MasterAtArmsGameState {
    const gameState = new MasterAtArmsGameState(parent);

    gameState.selectedRegions = data.selectedRegions.map((rid) =>
      parent.ingame.world.regions.get(rid)
    );
  ...
  deserializeChildGameState(
    data: SerializedMasterAtArmsGameState["childGameState"]
  ): MasterAtArmsGameState["childGameState"] {
