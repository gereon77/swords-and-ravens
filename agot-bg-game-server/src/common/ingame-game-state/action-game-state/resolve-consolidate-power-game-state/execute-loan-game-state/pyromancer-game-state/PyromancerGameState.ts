...
export default class PyromancerGameState extends GameState<
  ExecuteLoanGameState,
  SelectRegionGameState<PyromancerGameState> | SimpleChoiceGameState
> {
  ...
  onSelectRegionFinish(house: House, region: Region): void {
    ...
    this.setChildGameState(new SimpleChoiceGameState(this)).firstStart(
      house,
      "What kind of improvement do you choose?",
      ["Barrel", "Crown"]
    );
  }
  ...
  static deserializeFromServer(
    parent: ExecuteLoanGameState,
    data: SerializedPyromancerGameState
  ): PyromancerGameState {
    const gameState = new PyromancerGameState(parent);

    gameState.chosenRegion = data.chosenRegion
      ? parent.ingame.world.regions.get(data.chosenRegion)
      : null;
  ...
  deserializeChildGameState(
    data: SerializedPyromancerGameState["childGameState"]
  ): PyromancerGameState["childGameState"] {
    switch (data.type) {
      case "simple-choice":
