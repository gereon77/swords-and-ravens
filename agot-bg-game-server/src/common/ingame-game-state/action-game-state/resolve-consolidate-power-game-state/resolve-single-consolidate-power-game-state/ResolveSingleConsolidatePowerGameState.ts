...
export default class ResolveSingleConsolidatePowerGameState extends GameState<ResolveConsolidatePowerGameState> {
  ...
  firstStart(house: House): void {
    ...
    const availableOrders =
      this.parentGameState.getAvailableOrdersOfHouse(house).entries;
    ...
    const defenseMusterOrders = availableOrders.filter(
      ([_r, ot]) => ot instanceof DefenseMusterOrderType
    );
    const consolidatePowerOrders = new BetterMap(
      availableOrders.filter(
        ([_r, ot]) => ot instanceof ConsolidatePowerOrderType
      )
    );
    const ironBankOrders = availableOrders.filter(
      ([_r, ot]) => ot instanceof IronBankOrderType
    );

    if (
      defenseMusterOrders.length > 1 ||
      (defenseMusterOrders.length == 1 &&
        (consolidatePowerOrders.size > 0 || ironBankOrders.length > 0))
    ) {
      throw new Error(
        "Too much Defense / Muster orders placed or Defense / Muster together with player CP/IB orders placed!"
      );
    }
    ...
    if (
      this.canCpOrdersBeResolvedAutomatically(
        ironBankOrders,
        consolidatePowerOrders
      )
    ) {
      ...
      const regionsWithPossibleGains = consolidatePowerOrders.keys.map(
        (r) =>
      ...
      this.parentGameState.resolveConsolidatePowerOrderForPt(
    ...
    if (
      ironBankOrders.length == 1 &&
      consolidatePowerOrders.size == 0 &&
      this.ironBank?.getPurchasableLoans(this.house).length == 0
    ) {
    ...
    if (
      ...
      (this.ironBank?.loanSlots.every((lc) => lc == null) ||
        this.ingame.isVassalHouse(this.house))
    ) {
      const region = ironBankOrders[0][0];
      this.actionGameState.removeOrderFromRegion(
        region,
  ...
  onPlayerMessage(player: Player, message: ClientMessage): void {
    if (message.type == "resolve-consolidate-power-choice") {
      ...
      const regionOfOrder = this.ingame.world.regions.tryGet(
        message.region,
        null
      );
      if (!regionOfOrder) {
      ...
      const order = this.actionGameState.ordersOnBoard.tryGet(
        regionOfOrder,
        null
      );
      ...
      if (order.type instanceof ConsolidatePowerOrderType) {
        if (message.gainPowerTokens) {
          ...
          this.parentGameState
            .setChildGameState(
              new PlayerMusteringGameState(this.parentGameState)
            )
            .firstStart(
              this.house,
        ...
        this.parentGameState
          .setChildGameState(new PlayerMusteringGameState(this.parentGameState))
          .firstStart(this.house, PlayerMusteringType.DEFENSE_MUSTER_ORDER);
        return;
      } else if (
        order.type instanceof IronBankOrderType &&
        this.ironBank &&
        message.purchaseLoan !== undefined
      ) {
        if (this.ingame.isVassalHouse(this.house)) {
          return;
        }

        const loan = this.ironBank.purchaseLoan(
          this.house,
          message.purchaseLoan,
          regionOfOrder.id
        );
        if (!loan) {
          return;
  ...
  static deserializeFromServer(
    resolveConsolidatePower: ResolveConsolidatePowerGameState,
    data: SerializedResolveSingleConsolidatePowerGameState
  ): ResolveSingleConsolidatePowerGameState {
    const resolveSingleConsolidatePower =
      new ResolveSingleConsolidatePowerGameState(resolveConsolidatePower);
    resolveSingleConsolidatePower.house =
      resolveConsolidatePower.game.houses.get(data.house);
