...
export default class ResolveMarchOrderGameState extends GameState<
  ...
  | CallForSupportAgainstNeutralForceGameState
> {
  ...
  onResolveSingleMarchOrderGameStateFinish(house: House): void {
    ...
    const consequence =
      this.ingame.processPossibleConsequencesOfUnitLossAndCheckWinningConditions();
  ...
  getNextHouseToResolveMarchOrder(): House | null {
    ...
    for (let i = 0; i < numberOfPlayers; i++) {
      this.currentTurnOrderIndex =
        (this.currentTurnOrderIndex + 1) % numberOfPlayers;
  ...
  onPlayerMessage(player: Player, message: ClientMessage): void {
    if (message.type == "call-for-support-against-neutral-force") {
      const resolveSingleMarch =
        this.childGameState instanceof ResolveSingleMarchOrderGameState
          ? this.childGameState
          : null;
      if (
        !resolveSingleMarch ||
        this.ingameGameState.getControllerOfHouse(resolveSingleMarch.house) !=
          player
      ) {
        return;
      }
      ...
      const possipleSupporters =
        resolveSingleMarch.getPossibleSupportingHousesAgainstNeutralForces();
  ...
  static deserializeFromServer(
    ...
    resolveMarchOrderGameState.childGameState =
      resolveMarchOrderGameState.deserializeChildGameState(data.childGameState);
