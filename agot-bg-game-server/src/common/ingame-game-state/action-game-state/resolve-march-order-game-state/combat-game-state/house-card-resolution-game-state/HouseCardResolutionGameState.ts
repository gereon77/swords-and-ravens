...
export default class HouseCardResolutionGameState<
  P extends ParentGameState,
  C extends ChildGameState
> extends GameState<P, C> {
  ...
  resolveNextHouseCard(): void {
    ...
    const nextHouse = this.combatGameState
      .getOrderResolutionHouseCard()
  ...
  serializeToClient(
    admin: boolean,
    player: Player | null
  ): SerializedHouseCardResolutionGameState<any> {
    return {
  ...
  static deserializeFromServer(
    ...
    data: SerializedHouseCardResolutionGameState<any>
  ): HouseCardResolutionGameState<any, any> {
    const houseCardResolution = new HouseCardResolutionGameState(parent);

    houseCardResolution.resolvedHouses = data.resolvedHouses.map((hid) =>
      houseCardResolution.game.houses.get(hid)
    );
    houseCardResolution.childGameState =
      houseCardResolution.deserializeChildGameState(data.childGameState);
