...
> {
  ...
  firstStart(house: House): void {
    ...
    if (availableFootmen.length == 0) {
      ...
      this.ingame.log({
        ...
        house: house.id,
        houseCard: (
          this.combatGameState.houseCombatDatas.get(enemy)
      ...
      if (
        availableFootmen.every((fm) => !fm.wounded) ||
        availableFootmen.every((fm) => fm.wounded)
      ) {
        ...
        this.onSelectUnitsEnd(
          house,
          groupBy(selectedFootman, (u) => u.region).entries,
          true
        );
      } else {
  ...
  getAvailableFootmen(house: House): Unit[] {
    return this.combatGameState.houseCombatDatas
      .get(this.combatGameState.getEnemy(house))
      .army.filter((u) => u.type == footman);
  }
  ...
  static deserializeFromServer(
    ...
    data: SerializedMaceTyrellAbilityGameState
  ...
  deserializeChildGameState(
    data: SerializedMaceTyrellAbilityGameState["childGameState"]
  ): MaceTyrellAbilityGameState["childGameState"] {
