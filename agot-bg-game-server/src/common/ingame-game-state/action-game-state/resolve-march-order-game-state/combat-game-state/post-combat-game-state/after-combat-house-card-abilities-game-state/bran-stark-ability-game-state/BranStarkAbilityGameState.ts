...
> {
  ...
  onSelectHouseCardFinish(
    ...
    if (houseCard == null) {
      throw new Error(
        "BranStarkAbilityGameState does not allow houseCard to be null"
      );
    }
  ...
  static deserializeFromServer(
    ...
    data: SerializedBranStarkAbilityGameState
  ): BranStarkAbilityGameState {
    const branStarkAbilityGameState = new BranStarkAbilityGameState(
      afterCombat
    );

    branStarkAbilityGameState.childGameState =
      branStarkAbilityGameState.deserializeChildGameState(data.childGameState);

    return branStarkAbilityGameState;
  }

  deserializeChildGameState(
    data: SerializedBranStarkAbilityGameState["childGameState"]
  ):
    | SelectHouseCardGameState<BranStarkAbilityGameState>
