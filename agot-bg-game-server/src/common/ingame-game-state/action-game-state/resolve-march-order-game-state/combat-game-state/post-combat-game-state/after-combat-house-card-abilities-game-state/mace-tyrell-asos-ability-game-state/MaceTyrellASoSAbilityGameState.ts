...
import {
  defensePlusOne,
  support
} from "../../../../../../game-data-structure/order-types/orderTypes";
...
> {
  ...
  getAvailableOrdersOfHouse(house: House): Order[] {
    const placedOrders = new BetterMap(
      this.actionGameState.getOrdersOfHouse(house)
    );
    const available = _.uniqBy(
      this.ingame.game.getAvailableOrders(placedOrders, house),
      (o) => o.type
    );
    ...
    return _.orderBy(
      available.filter((o) => o.type == support || o.type == defensePlusOne),
      (o) => o.type.name,
      "desc"
    );
  }
  ...
  onSimpleChoiceGameStateEnd(
    ...
    if (!this.activated) {
      ...
      this.combatGameState.actionGameState.removeOrderFromRegion(
        this.combatGameState.defendingRegion,
        true
      );
      this.combatGameState.actionGameState.ordersOnBoard.set(
        this.combatGameState.defendingRegion,
        chosenOrder
      );
  ...
  static deserializeFromServer(
    afterWinnerDeterminationChild: AfterCombatHouseCardAbilitiesGameState["childGameState"],
    data: SerializedMaceTyrellASoSAbilityGameState
  ): MaceTyrellASoSAbilityGameState {
    const maceTyrellASoSAbilityGameState = new MaceTyrellASoSAbilityGameState(
      afterWinnerDeterminationChild
    );

    maceTyrellASoSAbilityGameState.activated = data.activated;
    maceTyrellASoSAbilityGameState.availableOrders = data.availableOrders.map(
      (oid) => orders.get(oid)
    );
    maceTyrellASoSAbilityGameState.childGameState =
      maceTyrellASoSAbilityGameState.deserializeChildGameState(
        data.childGameState
      );
  ...
  deserializeChildGameState(
    data: SerializedMaceTyrellASoSAbilityGameState["childGameState"]
  ): MaceTyrellASoSAbilityGameState["childGameState"] {
