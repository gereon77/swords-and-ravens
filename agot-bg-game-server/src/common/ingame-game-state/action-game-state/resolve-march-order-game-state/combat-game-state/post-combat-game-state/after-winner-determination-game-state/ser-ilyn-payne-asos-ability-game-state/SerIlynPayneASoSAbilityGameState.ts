...
export default class SerIlynPayneASoSAbilityGameState extends GameState<
  ...
  SelectUnitsGameState<SerIlynPayneASoSAbilityGameState>
> {
  house: House;
  get game(): Game {
  ...
  firstStart(house: House): void {
    ...
    if (this.combatGameState.areCasualtiesPrevented(this.enemy)) {
      this.ingame.log({
        ...
        house: this.enemy.id,
        houseCard: (
          this.combatGameState.houseCombatDatas.get(this.enemy)
            .houseCard as HouseCard
        ).id
      });
  ...
  onSelectUnitsEnd(
    ...
    selectedUnits.forEach(([region, units]) => {
      ...
      const houseCombatData = this.combatGameState.houseCombatDatas.get(
        this.enemy
      );
  ...
  static deserializeFromServer(
    ...
    data: SerializedSerIlynPayneASoSAbilityGameState
  ): SerIlynPayneASoSAbilityGameState {
    const serIlynPayneAbility = new SerIlynPayneASoSAbilityGameState(
      afterWinnerDeterminationChild
    );

    serIlynPayneAbility.house = afterWinnerDeterminationChild.game.houses.get(
      data.house
    );
    serIlynPayneAbility.childGameState =
      serIlynPayneAbility.deserializeChildGameState(data.childGameState);

    return serIlynPayneAbility;
  }

  deserializeChildGameState(
    data: SerializedSerIlynPayneASoSAbilityGameState["childGameState"]
  ): SerIlynPayneASoSAbilityGameState["childGameState"] {
    switch (data.type) {
      case "select-units":
