...
> {
  ...
  firstStart(): void {
    ...
    const overridenChooser = this.getOverridenRetreatLocationChooser(
      this.postCombat.loser
    );
    const finalChooser = overridenChooser
      ? overridenChooser
      : this.postCombat.loser;
  ...
  onSelectUnitsEnd(
    ...
    selectedUnits.forEach(([region, units]) => {
      ...
      this.postCombat.loserCombatData.army = _.difference(
        this.postCombat.loserCombatData.army,
        units
      );
  ...
  getOverridenRetreatLocationChooser(retreater: House): House | null {
    return this.combat.getOrderResolutionHouseCard().reduce((s, h) => {
      ...
      return houseCard.ability && !s
        ? houseCard.ability.overrideRetreatLocationChooser(
            this.postCombat,
            h,
            houseCard,
            retreater
  ...
  getValidRetreatRegions(
    ...
    return casualtiesPerRegion
      .filter(([_, c]) => c == lowestCasualty)
      .map(([r, _]) => r);
  }
  ...
  getCasualtiesOfRetreatRegion(retreatRegion: Region): number {
    ...
    const indexTooMuch = retreatingArmy.findIndex((_, i) => {
      ...
      return this.game.hasTooMuchArmies(
        ...
        new BetterMap([
          [this.postCombat.loserCombatData.region, retreatingArmy]
        ])
      );
    });
  ...
  static deserializeFromServer(
    ...
    data: SerializedResolveRetreatGameState
    ...
    resolveRetreat.retreatRegion = data.retreatRegion
      ...
      : null;
    resolveRetreat.childGameState = resolveRetreat.deserializeChildGameState(
      data.childGameState
    );
  ...
  deserializeChildGameState(
    data: SerializedResolveRetreatGameState["childGameState"]
  ): ResolveRetreatGameState["childGameState"] {
    switch (data.type) {
      case "select-region":
