...
import RenlyBaratheonAbilityGameState from "../../action-game-state/resolve-march-order-game-state/combat-game-state/post-combat-game-state/after-winner-determination-game-state/renly-baratheon-ability-game-state/RenlyBaratheonAbilityGameState";
  ...
  afterWinnerDetermination(
    ...
    if (afterWinnerDetermination.postCombatGameState.winner == house) {
      afterWinnerDetermination.childGameState
        .setChildGameState(
          new RenlyBaratheonAbilityGameState(
            afterWinnerDetermination.childGameState
          )
        )
