...
export default class PlanningGameState extends GameState<
  IngameGameState,
  | PlaceOrdersGameState
  | PlaceOrdersForVassalsGameState
...
> {
  ...
  @observable placedOrders: BetterMap<Region, Order | null> = new BetterMap<
    Region,
    Order | null
  >();
  ...
  onClaimVassalsFinished(): void {
    if (
      this.entireGame.gameSettings.precedingMustering &&
      this.game.turn == 1
    ) {
      this.ingame.log({
