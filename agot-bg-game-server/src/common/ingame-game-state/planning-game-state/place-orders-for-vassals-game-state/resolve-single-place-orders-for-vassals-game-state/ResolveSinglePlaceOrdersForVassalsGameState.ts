...
export default class ResolveSinglePlaceOrdersForVassalsGameState extends GameState<PlaceOrdersForVassalsGameState> {
  ...
  firstStart(house: House): void {
    ...
    const availableRegionsForOrders = this.ingame
      .getVassalsControlledByPlayer(player)
      .reduce(
        (count, h) =>
          count +
          this.placeOrdersGameState.getPossibleRegionsForOrders(h).length,
        0
      );
  ...
  onPlayerMessage(player: Player, message: ClientMessage): void {
    if (message.type == "place-order") {
      ...
      const house = _.find(
        this.placeOrdersGameState.getHousesToPutOrdersForPlayer(player),
        (h) =>
          this.placeOrdersGameState
            .getPossibleRegionsForOrders(h)
            .includes(region)
      );
      ...
      if (
        ...
        const availableOrders = this.placeOrdersGameState
          .getAvailableOrders(house)
          .filter((o) => o != order);
        if (
  ...
  serializeToClient(
    _admin: boolean,
    _player: Player | null
  ): SerializedSinglePlaceOrdersForVassalsGameState {
    return {
  ...
  actionAfterVassalReplacement(newVassal: House): void {
    ...
    const placedOrders =
      this.placeOrdersGameState.getPlacedOrdersOfHouse(newVassal);
    ...
    this.planningGameState
      .setChildGameState(new PlaceOrdersGameState(this.planningGameState))
      .firstStart();
  }

  static deserializeFromServer(
    placeOrdersForVassals: PlaceOrdersForVassalsGameState,
    data: SerializedSinglePlaceOrdersForVassalsGameState
  ): ResolveSinglePlaceOrdersForVassalsGameState {
    const singlePlaceOrder = new ResolveSinglePlaceOrdersForVassalsGameState(
      placeOrdersForVassals
    );

    singlePlaceOrder.house = placeOrdersForVassals.ingame.game.houses.get(
      data.house
    );
