...
export class ReplacePlayer extends VoteType {
  ...
  executeAccepted(vote: Vote): void {
    ...
    if (!ingame.oldPlayerIds.includes(oldPlayer.user._id)) {
      ingame.oldPlayerIds.push(oldPlayer.user._id);
    }

    if (!ingame.replacerIds.includes(newPlayer.user._id)) {
      ingame.replacerIds.push(newPlayer.user._id);
    }
