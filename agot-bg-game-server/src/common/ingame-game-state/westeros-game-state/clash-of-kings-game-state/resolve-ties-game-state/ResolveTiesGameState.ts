...
export default class ResolveTiesGameState extends GameState<ClashOfKingsGameState> {
  ...
  onPlayerMessage(player: Player, message: ClientMessage): void {
    if (message.type == "resolve-ties") {
      ...
      const resolvedTies = message.resolvedTies.map((hids) =>
        hids.map((hid) => this.game.houses.get(hid))
      );
      ...
      for (let i = 0; i < tiesToResolve.length; i++) {
        ...
        if (
          _.difference(resolvedTies[i], tiesToResolve[i].houses).length != 0
        ) {
          return;
        }
      }
      ...
      this.parentGameState.onResolveTiesGameState(
        this.bidResults,
        finalOrdering
      );
    }
  ...
  getWaitedUsers(): User[] {
    return [
      this.parentGameState.parentGameState.ingame.getControllerOfHouse(
        this.decider
      ).user
    ];
  }
  ...
  getTiesToResolve(): { trackerPlace: number; houses: House[] }[] {
    return this.bidResults
      .map(([_bid, houses], i) => {
        ...
        return {
          trackerPlace: _.sum(
            this.bidResults.slice(0, i).map(([_bid, houses]) => houses.length)
          ),
          houses: houses
        };
      })
  ...
  getBidOfHouse(house: House): number {
    const index = this.bidResults.findIndex(([_bid, houses]) =>
  ...
  serializeToClient(
    ...
    return {
      type: "resolve-ties",
      bidResults: this.bidResults.map(([bid, houses]) => [
        bid,
        houses.map((h) => h.id)
      ])
    };
  ...
  static deserializeFromServer(
    clashOfKings: ClashOfKingsGameState,
    data: SerializedResolveTiesGameState
  ): ResolveTiesGameState {
    const resolveTies = new ResolveTiesGameState(clashOfKings);

    resolveTies.bidResults = data.bidResults.map(([bid, houseIds]) => [
      bid,
      houseIds.map((hid) => clashOfKings.game.houses.get(hid))
    ]);
