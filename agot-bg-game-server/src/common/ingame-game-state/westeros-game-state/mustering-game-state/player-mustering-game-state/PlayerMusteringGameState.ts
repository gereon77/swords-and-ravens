...
import ResolveConsolidatePowerGameState from "../../../action-game-state/resolve-consolidate-power-game-state/ResolveConsolidatePowerGameState";
...
export default class PlayerMusteringGameState extends GameState<ParentGameState> {
  ...
  get regions(): Region[] {
    switch (this.type) {
      ...
      case PlayerMusteringType.DEFENSE_MUSTER_ORDER:
        return this.resolveConsolidatePowerGameState.actionGameState
          .getRegionsWithDefenseMusterOrderOfHouse(this.house)
          .map(([r, _ot]) => r);
      case PlayerMusteringType.STARRED_CONSOLIDATE_POWER:
        return this.resolveConsolidatePowerGameState.actionGameState
          .getRegionsWithConsolidatePowerOrderOfHouse(this.house)
          .filter(([_r, ot]) => ot.starred)
          .map(([r, _ot]) => r);
  ...
  onPlayerMessage(player: Player, message: ClientMessage): void {
    if (message.type == "muster") {
      ...
      if (
        (this.type == PlayerMusteringType.MUSTERING_WESTEROS_CARD ||
          this.type == PlayerMusteringType.RALLY_THE_MEN_WESTEROS_CARD) &&
        this.ingame.isVassalHouse(this.house)
      ) {
        if (
          musterings.keys.some((r) => r.superControlPowerToken != this.house)
        ) {
          return;
        }
      }
      ...
      for (const [originatingRegion, recruitements] of musterings.entries) {
        ...
        const transformedUnits = recruitements
          .filter(({ from }) => from)
          .map(({ from }) => from);
  ...
  isMusteringValid(musterings: BetterMap<Region, Mustering[]>): boolean {
    ...
    // ...
    return musterings.entries.every(
      ([originatingRegion, recruitements], mi) => {
    ...
    switch (this.type) {
      ...
      case PlayerMusteringType.THE_HORDE_DESCENDS_WILDLING_CARD:
        if (musterings.size == 0) {
          ...
          return (
            this.getValidMusteringRulesForRegion(musterings.keys[0], musterings)
              .length > 0
          );
        }
    }
