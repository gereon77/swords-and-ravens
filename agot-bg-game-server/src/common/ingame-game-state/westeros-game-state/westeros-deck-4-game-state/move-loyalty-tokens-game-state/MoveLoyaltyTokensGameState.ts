...
import ResolveMoveLoyaltyTokenGameState, {
  SerializedResolveMoveLoyaltyTokenGameState
} from "./resolve-move-loyalty-token-game-state/ResolveMoveLoyaltyTokenGameState";
...
export default class MoveLoyaltyTokensGameState extends GameState<
  WesterosDeck4GameState,
  SimpleChoiceGameState | ResolveMoveLoyaltyTokenGameState
> {
  resolveOrder: House[];
  ...
  get validFromRegions(): Region[] {
    return this.game.world.regions.values
      .filter((r) => r.loyaltyTokens > 0)
      .filter((r) => this.getValidTargetRegions(r).length > 0);
  }
  ...
  proceedNextResolve(): void {
    ...
    this.setChildGameState(
      new ResolveMoveLoyaltyTokenGameState(this)
    ).firstStart(nextHouse);
  }
  ...
  getValidTargetRegions(regionFrom: Region): Region[] {
    return this.game.world
      .getNeighbouringRegions(regionFrom)
  ...
  getChoices(house: House): string[] {
    ...
    result.push(
      `Discard ${this.costsToCancelPreviousMovement} Power token${this.costsToCancelPreviousMovement != 1 ? "s" : ""} to cancel the previous movement`
    );
    return result;
  }
  ...
  onSimpleChoiceGameStateEnd(
    ...
    if (choice == 0) {
      ...
      // Undo the last move
      this.moveLoyaltyToken(
        this.previousMovement.to,
        this.previousMovement.from
      );
    }
  ...
  onPlayerMessage(player: Player, message: ClientMessage): void {
    ...
    if (
      message.type == "change-accept-all-loyalty-token-movements" &&
      this.ingame.getControllerOfHouse(this.game.targaryen) == player
    ) {
  ...
  serializeToClient(
    admin: boolean,
    player: Player | null
  ): SerializedMoveLoyaltyTokensGameState {
    ...
    return {
      ...
      previousMovement: this.previousMovement
        ...
        : null,
      acceptAllMovements:
        admin || playerControlsTargaryen ? this.acceptAllMovements : false,
      childGameState:
        this.childGameState.serializeToClient(admin, player) ?? null
    };
  }

  static deserializeFromServer(
    westerosDeck4: WesterosDeck4GameState,
    data: SerializedMoveLoyaltyTokensGameState
  ): MoveLoyaltyTokensGameState {
    const gameState = new MoveLoyaltyTokensGameState(westerosDeck4);

    gameState.resolveOrder = data.resolveOrder.map((hid) =>
      westerosDeck4.game.houses.get(hid)
    );
    gameState.costsToCancelPreviousMovement =
      data.costsToCancelPreviousMovement;
    gameState.previousMovement = data.previousMovement
      ? {
          house: westerosDeck4.game.houses.get(data.previousMovement.house),
          from: westerosDeck4.game.world.regions.get(
            data.previousMovement.from
          ),
          to: westerosDeck4.game.world.regions.get(data.previousMovement.to)
        }
      : null;
    gameState.acceptAllMovements = data.acceptAllMovements;
    gameState.childGameState = gameState.deserializeChildGameState(
      data.childGameState
    );
  ...
  deserializeChildGameState(
    data: SerializedMoveLoyaltyTokensGameState["childGameState"]
  ): MoveLoyaltyTokensGameState["childGameState"] {
