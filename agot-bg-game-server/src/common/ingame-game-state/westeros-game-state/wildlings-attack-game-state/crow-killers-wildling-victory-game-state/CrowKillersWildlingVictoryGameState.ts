...
export default class CrowKillersWildlingVictoryGameState extends WildlingCardEffectInTurnOrderGameState<
  | SelectUnitsGameState<CrowKillersWildlingVictoryGameState>
...
> {
  ...
  executeForLowestBidder(house: House): void {
    ...
    const knightsToTransform = this.game.world
      ...
      .map(
        (r) =>
          [r, r.units.values.filter((u) => u.type == knight)] as [
            Region,
            Unit[]
          ]
      );

    const flattenedKnights = _.flatMap(
      knightsToTransform,
      ([_region, units]) => units
    );
    const availableFootmanCount = this.game.getAvailableUnitsOfType(
      house,
      footman
    );
    ...
    if (flattenedKnights.length <= availableFootmanCount) {
      ...
      this.setChildGameState(new SelectUnitsGameState(this)).firstStart(
        house,
        flattenedKnights,
        flattenedKnights.length - availableFootmanCount
      );
    }
  }
  ...
  getSelectableKnights(house: House): Unit[] {
    return _.flatMap(
      this.game.world.getControlledRegions(house).map((r) => r.units.values)
  ...
  destroySelection(house: House, selectedUnits: [Region, Unit[]][]): number {
    ...
    this.ingame.log({
      ...
      house: house.id,
      units: selectedUnits.map(([region, knights]) => [
        region.id,
        knights.map((k) => k.type.id)
      ])
    });
  ...
  transformSelection(
    ...
    selectedUnits.forEach(([region, knights]) =>
      this.ingame.transformUnits(region, knights, footman)
    );
  ...
  onTakeControlOfEnemyPortGameStateRequired(
    takeOver: TakeOverPort,
    previousHouse: House
  ): void {
    this.setChildGameState(
      new TakeControlOfEnemyPortGameState(this)
    ).firstStart(takeOver.port, takeOver.newController, previousHouse);
  }
  ...
  static deserializeFromServer(
    wildlingsAttack: WildlingsAttackGameState,
    data: SerializedCrowKillersWildlingVictoryGameState
  ): CrowKillersWildlingVictoryGameState {
    const crowKillers = new CrowKillersWildlingVictoryGameState(
      wildlingsAttack
    );

    crowKillers.childGameState = crowKillers.deserializeChildGameState(
      data.childGameState
    );
  ...
  deserializeChildGameState(
    data: SerializedCrowKillersWildlingVictoryGameState["childGameState"]
  ): CrowKillersWildlingVictoryGameState["childGameState"] {
    switch (data.type) {
      case "select-units":
        return SelectUnitsGameState.deserializeFromServer(this, data);
      case "take-control-of-enemy-port":
        return TakeControlOfEnemyPortGameState.deserializeFromServer(
          this,
          data
        );
    }
