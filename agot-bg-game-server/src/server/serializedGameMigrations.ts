...
const serializedGameMigrations: {
  version: string;
  migrate: (serializeGamed: any) => any;
}[] = [
  {
    version: "2",
    ...
    migrate: (serializedGame: any) => {
      ...
      if (serializedGame.childGameState.type == "ingame") {
        ...
        if (serializedIngameGameState.childGameState.type == "action") {
          const serializedActionGameState =
            serializedIngameGameState.childGameState;
          if (
            serializedActionGameState.childGameState.type ==
            "resolve-march-order"
          ) {
            const serializedResolveMarchOrderGameState =
              serializedActionGameState.childGameState;

            let lastSelectedId: any;

            const serializedChildGameState =
              serializedResolveMarchOrderGameState.childGameState;
            switch (serializedChildGameState.type) {
              ...
              case "take-control-of-enemy-port":
                lastSelectedId =
                  serializedChildGameState.lastHouseThatResolvedMarchOrderId;
                break;
              default:
      ...
      if (serializedGame.childGameState.type == "ingame") {
        ...
        ingame.gameLogManager.logs
          ...
          .forEach((log: any) => {
            ...
            const cannotRetreatNames: string[] =
              log.data.killedBecauseCantRetreat;
        ...
        ingame.gameLogManager.logs
          ...
          .forEach((log: any) => {
            ...
            const portName = log.data.port;
            log.data.oldController =
              houseNameToIdMappings.get(oldControllerName);
            log.data.newController =
              houseNameToIdMappings.get(newControllerName);
            log.data.port = regionNameToIdMappings.get(portName);
          });
    ...
    migrate: (serializedGame: any) => {
      ...
      if (serializedGame.childGameState.type == "ingame") {
        ...
        if (ingame.childGameState && ingame.childGameState.type == "westeros") {
          ...
          if (
            westeros.childGameState &&
            westeros.childGameState.type == "wildlings-attack"
          ) {
            const wildlingsAttack = westeros.childGameState;

            if (
              wildlingsAttack.childGameState &&
              wildlingsAttack.childGameState == "crow-killers-wildling-victory"
            ) {
              ...
              crowKillersWildlingVictory.step =
                CrowKillersStep.DEGRADING_KNIGHTS;
            }
          }
        }
      }
    ...
    migrate: (serializedGame: any) => {
      ...
      if (serializedGame.childGameState.type == "ingame") {
        ...
        if (ingame.childGameState && ingame.childGameState.type == "action") {
          ...
          if (
            action.childGameState &&
            action.childGameState.type == "use-raven"
          ) {
            const useRaven = action.childGameState;
            if (
              useRaven.childGameState &&
              useRaven.childGameState.type == "choose-raven-action"
            ) {
    ...
    migrate: (serializedGame: any) => {
      ...
      if (serializedGame.childGameState.type == "ingame") {
        ...
        if (ingame.childGameState && ingame.childGameState.type == "action") {
          ...
          if (
            action.childGameState &&
            action.childGameState.type == "resolve-march-order"
          ) {
            const resolveMarchOrders = action.childGameState;
            if (
              resolveMarchOrders.childGameState &&
              resolveMarchOrders.childGameState.type == "combat"
            ) {
              const combat = resolveMarchOrders.childGameState;
              if (
                combat.childGameState &&
                combat.childGameState.type ==
                  "immediately-house-card-abilities-resolution"
              ) {
                const immediatelyHouseCardResolution = combat.childGameState;
                if (
                  immediatelyHouseCardResolution.childGameState &&
                  (immediatelyHouseCardResolution.childGameState.type ==
                    "aeron-damphair-dwd-ability" ||
                    immediatelyHouseCardResolution.childGameState.type ==
                      "qyburn-ability")
                ) {
                  // Aron and Qyburn are now BeforeCombat states => Convert combat.childGameState to BeforeCombat
                  combat.childGameState.type =
                    "before-combat-house-card-abilities-resolution";
                }
              }
            }
          }
        ...
        if (ingame.childGameState && ingame.childGameState.type == "action") {
          ...
          if (
            ...
            if (
              ...
              if (
                combat.childGameState &&
                combat.childGameState.type == "post-combat"
              ) {
                const postCombat = combat.childGameState;
                if (
                  postCombat.childGameState &&
                  postCombat.childGameState.type ==
                    "after-combat-house-card-abilities"
                ) {
                  const afterCombatHouseCardAbilities =
                    postCombat.childGameState;
                  if (
                    afterCombatHouseCardAbilities.childGameState &&
                    afterCombatHouseCardAbilities.childGameState.type ==
                      "rodrik-the-reader-ability"
                  ) {
          ...
          if (
            ...
            if (
              ...
              if (
                ...
                if (
                  beforeCombatResultion.childGameState &&
                  beforeCombatResultion.childGameState.type ==
                    "aeron-damphair-dwd-ability"
                ) {
  ...
  },
  {
    ...
    migrate: (serializedGame: any) => {
      ...
      if (serializedGame.childGameState.type == "ingame") {
        const ingame = serializedGame.childGameState;
        if (
          serializedGame.gameSettings &&
          serializedGame.gameSettings.setupId == "a-dance-with-dragons"
        ) {
      ...
      if (serializedGame.childGameState.type == "ingame") {
        ...
        if (ingame.childGameState.type == "planning") {
          ...
          if (planning.childGameState.type == "place-orders") {
            ...
            const vassalHouses = new BetterMap(ingame.game.vassalRelations)
              .keys as string[];
        ...
        if (
          ingame.childGameState.type == "action" &&
          ingame.childGameState.childGameState.type == "resolve-march-order" &&
          ingame.childGameState.childGameState.childGameState.type ==
            "combat" &&
          ingame.childGameState.childGameState.childGameState.childGameState
            .type == "before-combat-house-card-abilities-resolution" &&
          ingame.childGameState.childGameState.childGameState.childGameState
            .childGameState.type == "house-card-resolution"
        ) {
          const houseCardResolution =
            ingame.childGameState.childGameState.childGameState.childGameState
              .childGameState;
          if (
            houseCardResolution.childGameState.type ==
            "aeron-damphair-dwd-ability"
          ) {
    ...
    migrate: (serializedGame: any) => {
      ...
      if (serializedGame.childGameState.type == "ingame") {
        ...
        if (
          ...
          if (combat.childGameState.type == "post-combat") {
            const postCombat =
              ingame.childGameState.childGameState.childGameState
                .childGameState;
  ...
  },
  {
    ...
    migrate: (serializedGame: any) => {
      if (serializedGame.childGameState.type == "ingame") {
        const ingame = serializedGame.childGameState;
        ingame.game.clientNextWildlingCardId =
          ingame.game.clientNextWidllingCardId;
      }
  ...
  },
  {
    ...
    migrate: (serializedGame: any) => {
      ...
      if (serializedGame.childGameState.type == "ingame") {
        serializedGame.childGameState.game.deletedHouseCards =
          serializedGame.childGameState.game.replacedPlayerHouseCards;
      }
            ...
            if (resolveMarch.childGameState.type == "combat") {
              ...
              if (combat.childGameState.type == "post-combat") {
                ...
                if (
                  ...
                  if (
                    afterCombatAbilities.childGameState.childGameState.type ==
                    "robert-arryn-ability"
                  ) {
                    const robertArryn =
                      afterCombatAbilities.childGameState.childGameState;
        ...
        if (ingame.game.houses.length == 8) {
          ...
          ingame.gameLogManager.logs.forEach((l: any) => {
            if (l.data.type == "place-loyalty-choice") {
              l.data.loyaltyTokenCount =
                l.data.discardedPowerTokens == 0
                  ? 0
                  : l.data.discardedPowerTokens == 1 ||
                      l.data.discardedPowerTokens == 2
                    ? 1
                    : 2;
            }
          });
    ...
    migrate: (serializedGame: any) => {
      if (serializedGame.childGameState.type == "ingame") {
        ...
        if (ingame.childGameState.type == "westeros") {
          const westeros = ingame.childGameState;
          if (
            ingame.game.houses.length == 8 &&
            westeros.revealedCardIds.length == 3 &&
            westeros.childGameState.type != "westeros-deck-4"
          ) {
  ...
  },
  {
    ...
    migrate: (serializedGame: any) => {
      ...
      if (serializedGame.childGameState.type == "ingame") {
        ...
        if (
          ingame.childGameState.type == "westeros" &&
          ingame.childGameState.childGameState.type == "westeros-deck-4" &&
          ingame.childGameState.childGameState.childGameState.type ==
            "move-loyalty-tokens"
        ) {
          const moveLoyaltyTokens =
            ingame.childGameState.childGameState.childGameState;
          moveLoyaltyTokens.costsToCancelPreviousMovement =
            moveLoyaltyTokens.costsToDiscardChoice;
          ...
          if (moveLoyaltyTokens.childGameState.type == "select-region") {
            moveLoyaltyTokens.childGameState.type =
              "resolve-move-loyalty-token";
  ...
  },
  {
    ...
    migrate: (serializedGame: any) => {
      ...
      if (serializedGame.childGameState.type == "ingame") {
        ...
        if (
          ingame.childGameState.type == "action" &&
          ingame.childGameState.childGameState.type ==
            "resolve-consolidate-power" &&
          ingame.childGameState.childGameState.childGameState.type ==
            "player-mustering"
        ) {
        ...
        if (
          ...
          ingame.childGameState.childGameState.type == "westeros-deck-4" &&
          ingame.childGameState.childGameState.childGameState.type ==
            "choose-multiple-regions-for-loyalty-token"
        ) {
  ...
  },
  {
    ...
    migrate: (serializedGame: any) => {
      if (
        ...
        serializedGame.childGameState.childGameState.type == "game-ended"
      ) {
        const ingame = serializedGame.childGameState;
  ...
  },
  {
    ...
    migrate: (serializedGame: any) => {
      ...
      if (serializedGame.childGameState.type == "ingame") {
        ...
        if (
          ingame.childGameState.type == "westeros" &&
          ingame.childGameState.childGameState.type == "mustering"
        ) {
  ...
  },
  {
    ...
    migrate: (serializedGame: any) => {
      if (serializedGame.childGameState.type == "ingame") {
        const ingame = serializedGame.childGameState;
        ingame.game.victoryPointsCountNeededToWin =
          ingame.game.structuresCountNeededToWin;
        ingame.gameLogManager.logs.forEach((l: any) => {
          switch (l.data.type) {
      ...
      if (serializedGame.childGameState.type == "ingame") {
        ...
        ingame.gameLogManager.logs.forEach((l: any, i: number) => {
          ...
          if (l.data.type == "march-resolved" && l.data.moves.length == 0) {
            if (
              i + 1 < ingame.gameLogManager.logs.length &&
              ingame.gameLogManager.logs[i + 1].data.type == "attack"
            ) {
  ...
  },
  {
    ...
    migrate: (serializedGame: any) => {
      if (serializedGame.childGameState.type == "ingame") {
        ...
        if (
          ingame.childGameState.type == "action" &&
          ingame.childGameState.childGameState.type == "score-objectives" &&
          ingame.childGameState.childGameState.childGameState.type ==
            "score-other-objectives"
        ) {
          const scoreOtherObjectives =
            ingame.childGameState.childGameState.childGameState;
  ...
  },
  {
    ...
    migrate: (serializedGame: any) => {
      ...
      if (
        serializedGame.childGameState.type == "ingame" &&
        serializedGame.gameSettings.faceless
      ) {
        const ingame = serializedGame.childGameState;
  ...
  },
  {
    ...
    migrate: (serializedGame: any) => {
      if (serializedGame.childGameState.type == "lobby") {
        ...
        if (
          serializedGame.gameSettings.playerCount >= 8 ||
          serializedGame.gameSettings.startWithSevenPowerTokens
        ) {
  ...
  },
  {
    ...
    migrate: (serializedGame: any) => {
      if (serializedGame.gameSettings.setupId == "a-feast-for-crows") {
        ...
        if (
          serializedGame.childGameState.type == "ingame" &&
          serializedGame.childGameState.game.turn <= 10
        ) {
  ...
  },
  {
    ...
    migrate: (serializedGame: any) => {
      if (
        serializedGame.childGameState.type == "ingame" &&
        serializedGame.childGameState.childGameState.type ==
          "thematic-draft-house-cards"
      ) {
        const ingame = serializedGame.childGameState;
  ...
  },
  {
    ...
    migrate: (serializedGame: any) => {
      if (
        serializedGame.childGameState.type == "ingame" &&
        serializedGame.childGameState.childGameState.type == "westeros"
      ) {
        ...
        if (
          westeros.childGameState.type == "westeros-deck-4" &&
          westeros.childGameState.childGameState.type ==
            "choose-multiple-regions-for-loyalty-token"
        ) {
      ...
      if (serializedGame.childGameState.type == "ingame") {
        ...
        if (
          ...
          if (
            combat.childGameState.type ==
              "immediately-house-card-abilities-resolution" &&
            combat.childGameState.childGameState.type ==
          ...
          if (
            ...
            if (
              afterWinner.childGameState.type == "house-card-resolution" &&
              (afterWinner.childGameState.childGameState.type ==
                "alayne-stone-ability" ||
                afterWinner.childGameState.childGameState.type ==
                  "lysa-arryn-mod-ability" ||
                afterWinner.childGameState.childGameState.type ==
                  "reek-ability")
            ) {
  ...
  },
  {
    ...
    migrate: (serializedGame: any) => {
      if (
        serializedGame.childGameState.type == "ingame" &&
        serializedGame.childGameState.childGameState.type == "planning"
      ) {
  ...
  },
  {
    ...
    migrate: (serializedGame: any) => {
      if (serializedGame.childGameState.type == "ingame") {
        const ingame = serializedGame.childGameState;
        if (
          serializedGame.gameSettings.onlyLive &&
          !serializedGame.gameSettings.initialLiveClock
        ) {
          serializedGame.gameSettings.initialLiveClock = 75;
        }

        ingame.paused = ingame.game.paused ? ingame.game.paused : null;
        ingame.willBeAutoResumedAt = ingame.game.willBeAutoResumedAt
          ? ingame.game.willBeAutoResumedAt
          : null;
  ...
  },
  {
    ...
    migrate: (serializedGame: any) => {
      if (
        serializedGame.childGameState.type == "ingame" &&
        serializedGame.gameSettings.pbem
      ) {
        const ingame = serializedGame.childGameState;
  ...
  },
  {
    ...
    migrate: (serializedGame: any) => {
      if (
        serializedGame.childGameState.type == "ingame" &&
        serializedGame.gameSettings.setupId == "a-dance-with-mother-of-dragons"
      ) {
        const ingame = serializedGame.childGameState;
    ...
    migrate: (serializedGame: any) => {
      if (
        ...
        if (ingame.childGameState.type == "westeros") {
          ...
          westeros.revealAndResolveTop2WesterosDeck4Cards =
            westeros.revealAndResolveTop3WesterosDeck4Cards;
        }
      }
      return serializedGame;
    }
    ...
    migrate: (serializedGame: any) => {
      if (serializedGame.childGameState.type == "ingame") {
        ...
        if (
          ...
          ingame.childGameState.childGameState.type == "resolve-march-order" &&
          ingame.childGameState.childGameState.childGameState.type ==
            "take-control-of-enemy-port"
        ) {
          const takeControlOfEnemyPort =
            ingame.childGameState.childGameState.childGameState;
          takeControlOfEnemyPort.previousHouseId =
            takeControlOfEnemyPort.lastHouseThatResolvedMarchOrderId;
        }
      }
      return serializedGame;
    }
  ...
  },
  {
    ...
    migrate: (serializedGame: any) => {
      if (serializedGame.childGameState.type == "ingame") {
        const ingame = serializedGame.childGameState;
        if (
          ingame.childGameState.type != "draft-house-cards" &&
          ingame.childGameState.type != "thematic-draft-house-cards"
        ) {
          ingame.game.houseCardsForDrafting = [];
        }
      }
  ...
  },
  {
    version: "97",
    migrate: (serializedGame: any) => {
      serializedGame.gameSettings.customBalancing = serializedGame.gameSettings
        .customModBalancing
        ? true
        : false;
      return serializedGame;
  ...
  },
  {
    ...
    migrate: (serializedGame: any) => {
      ...
      if (
        serializedGame.childGameState.type == "ingame" &&
        serializedGame.gameSettings.setupId != "a-feast-for-crows"
      ) {
        const ingame = serializedGame.childGameState;
        ingame.game.houses.forEach((h: any) => {
  ...
  },
  {
    ...
    migrate: (serializedGame: any) => {
      if (serializedGame.childGameState.type == "ingame") {
        const ingame = serializedGame.childGameState;
        ingame.game.loyaltyTokenCountNeededToWin =
          ingame.game.victoryPointsCountNeededToWin;
      }
  ...
  },
  {
    ...
    migrate: (serializedGame: any) => {
      if (serializedGame.childGameState.type == "ingame") {
        serializedGame.gameSettings.loyaltyTokenCountNeededToWin =
          serializedGame.gameSettings.victoryPointsCountNeededToWin;
      }
  ...
  },
  {
    ...
    migrate: (serializedGame: any) => {
      ...
      if (
        ...
        if (
          game.removedDragonStrengthToken == 0 &&
          _.isEqual([2, 4, 6], _.sortBy(game.dragonStrengthTokens))
        ) {
  ...
  },
  {
    version: "111",
    migrate: (serializedGame: any) => {
      serializedGame.gameSettings.randomDraft =
  ...
  },
  {
    ...
    migrate: (serializedGame: any) => {
      if (serializedGame.childGameState.type == "ingame") {
        ...
        if (
          ingame.game.maxTurns == 6 &&
          ingame.game.dragonStrengthTokens.length > 0
        ) {
  ...
  },
  {
    ...
    migrate: (serializedGame: any) => {
      ...
      function applyMigrations(houseCards: [string, any][]): [string, any][] {
        ...
        // Fix typo in nerfed:
        result
          .filter(([id, _shc]: any) => id.endsWith("-nerved"))
          .forEach(([id, shc]: any) => {
