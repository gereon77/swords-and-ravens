...
namespace agot_bg_website.Areas.Identity.Pages.Account
{
    ...
    public class ExternalLoginModel : PageModel
    {
        ...
        )
        {
            ...
            else
            {
                ...
                if (info.Principal.HasClaim(c => c.Type == ClaimTypes.Email))
                {
                    ...
                    // Try linking onto an existing account *before* ever showing the "choose a
                    // username" registration form below: that form's username field is pointless
                    // and confusing when this verified email already belongs to an existing
                    // account, since the existing account's username is always kept as-is either
                    // way (TryAutoLinkExistingAccountAsync never touches it). Only a genuinely new
                    // email falls through to the form.
                    var autoLinkResult = await TryAutoLinkExistingAccountAsync(
                        info,
                        _userManager.NormalizeEmail(email),
                        returnUrl
                    );
                    if (autoLinkResult != null)
                    {
        ...
        /// <summary>
        /// Attempts to link the given external login onto an existing (previously-imported,
        /// unclaimed) account by email and sign the user in immediately - see
        /// AccountLinkingService for why only AccountLinkOutcome.Linked is safe to auto-merge
        /// into. Shared by both the initial GET callback (so an existing account never even sees
        /// the "choose a username" form) and the POST confirmation (in case the email changed, or
        /// was only supplied there, between the two requests).
        /// Returns null when there's no existing account to link to (AccountLinkOutcome.NoMatch),
        /// so the caller should fall through to showing/validating the registration form instead.
        /// </summary>
        private async Task<IActionResult> TryAutoLinkExistingAccountAsync(
        ...
        )
        {
            ...
            if (linkResult.Outcome == AccountLinkOutcome.NoMatch)
            {
                return null;
            }
            ...
            var existingUser = linkResult.User;
        ...
        public async Task<IActionResult> OnPostConfirmationAsync(string returnUrl = null)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                // This is normally already handled by OnGetCallbackAsync before the form is ever
                // shown - reaching here with a linkable email is the fallback for the rare case
                // where AddLoginAsync failed there and the user just resubmitted the form.
