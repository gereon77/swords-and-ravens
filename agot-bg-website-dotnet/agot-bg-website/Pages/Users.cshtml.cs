    ...
    : PageModel
{
    ...
    {
        if (
            !(await authorizationService.AuthorizeAsync(User, GamePermissions.ManageUserStatus)).Succeeded
        )
        {
            return Forbid();
        }

        if (role != RoleNames.Banned && role != RoleNames.OnProbation && role != RoleNames.Tongueless)
        {
            return BadRequest();
        }
